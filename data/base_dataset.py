    ...
    if opt.preprocess == 'resize_and_crop':
        if opt.source_size_count == 1:
            ...
            if not opt.isTrain:
                ...
                if opt.load_size == 128:
                    new_h = new_w = random.choice([148, 158, 168, 178])
                elif opt.load_size < 512:
                    new_h = new_w = random.choice([286, 306, 326, 346])
                elif opt.load_size >= 512 and opt.load_size <1024:
                    new_h = new_w = random.choice([526, 566, 606, 646])
                elif opt.load_size >= 1024 and opt.load_size <1536:
                    new_h = new_w = random.choice([1026, 1106, 1186, 1268])
                elif opt.load_size >= 1536 and opt.load_size <2048:
                    new_h = new_w = random.choice([1540, 1620, 1700, 1780])
                elif opt.load_size >=2048:
                    new_h = new_w = random.choice([2050, 2210, 2370, 2530])
