...
class UnetD2Generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        """
        super(UnetD2Generator, self).__init__()
        assert num_downs == 8
    ...
    def forward(self, x):
        ...
        # # upsample
        u8 = self.h_up8(d8)
        u7 = self.h_up7(torch.cat([d7, u8], 1))
        u6 = self.h_up6(torch.cat([d6, u7], 1))
        u5 = self.h_up5(torch.cat([d5, u6], 1))
        u4 = self.h_up4(torch.cat([d4, u5], 1))
        u3 = self.h_up3(torch.cat([d3, u4], 1))
        u2 = self.h_up2(torch.cat([d2, u3], 1))
        u1 = self.h_up1(torch.cat([d1, u2], 1))
        # return u1
        return h_u1, u1
